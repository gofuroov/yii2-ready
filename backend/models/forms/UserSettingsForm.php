...
class UserSettingsForm extends \yii\base\Model
{
    ...
    public function upload(): bool
    {
        ...
        if (FileHelper::createDirectory(\Yii::getAlias("@backend-uploads")) && $this->photo->saveAs("@backend-uploads/" . $photoName)) {

            //Delete old image if exists
            if ($this->user->photo) {
                $oldImagePath = \Yii::getAlias("@backend-uploads") . '/' . pathinfo($this->user->photo)['basename'];
                if (file_exists($oldImagePath)) {
                    unlink($oldImagePath);
                }
            }
            if ($this->photo->extension !== 'svg') {
                [$width, $height] = getimagesize(\Yii::getAlias("@backend-uploads") . '/' . $photoName);
                $minValue = min($width, $height);

                $imagine = new Imagine();
                $photo = $imagine->open(\Yii::getAlias("@backend-uploads") . '/' . $photoName);
                $photo->crop(new Point(0, 0), new Box($minValue, $minValue))->resize(new Box(500, 500))->save(\Yii::getAlias("@backend-uploads") . '/' . $photoName, [
                    'jpeg_quality' => 50
                ]);
            }
